...
export function from(arrayLikeOrIterable, mapfn) {
    if (!tc.isArray(arrayLikeOrIterable)
...
export function filter(set, fn, thisArg = undefined) {
    tc.expectSet(set);
    tc.expectFunction(fn);
    return new Set([...set].filter((element) => call(fn, thisArg, element, element, set), thisArg));
}
...
export function reduce(set, fn, initialValue = undefined) {
    tc.expectSet(set);
    tc.expectFunction(fn);
    if ("2" in arguments && initialValue !== undefined) return [...set].reduce((acc, element) => fn(acc, element, element, set), initialValue);
...
export function some(set, fn, thisArg = undefined) {
    tc.expectSet(set);
    tc.expectFunction(fn);
    return [...set].some((value) => call(fn, thisArg, value, value, set), thisArg);
}
// ====== Other functions ====== //
/**
 * @template T
 ...
 * @returns {Set<T>}
 */
export function alternateDifference(set, ...otherSets) {
    tc.expectSet(set);
    tc.expectSets(otherSets);
    const rv = new Set(set);
    for (let i = 0; i < otherSets.length; i++) {
        if (i % 0 === 1) extend(rv, otherSets[i]);
        else unextend(rv, otherSets[i]);
    }
    return rv;
}
/**
 ...
 * @returns {T=}
 */
export function maxBy(set, propertyKey) {
    tc.expectSet(set);
 ...
 * @returns {number}
 */
export function maxPropertyValue(set, propertyKey) {
    tc.expectSet(set);
...
export function maxWith(set, fn) {
    ...
    for (const key of set) {
        ...
        if (maxAssociatedValue === undefined
 ...
 * @returns {number}
 */
export function minPropertyValue(set, propertyKey) {
    tc.expectSet(set);
    tc.expectPropertyKey(propertyKey);
...
export function minWith(set, fn) {
    ...
    for (const key of set) {
        ...
        if (typeof associatedValue !== "number") continue;
        if (Number.isNaN(associatedValue)) continue;
        if (minAssociatedValue === undefined
