...
export function equals(object, otherObject, { allowUndefined = false } = {}) {
    ...
    if (Array.isArray(object)) {
        ...
        const length = Math.min(array.length, otherArray.length);
        for (let i = 0; i < length; i++) {
            const element = array[i];
            const otherElement = otherArray[i];
            if (tc.isPrimitive(element)) {
                if (!Object.is(element, otherElement)) return false;
            } else if (!equals(element, otherElement, copiedOptions)) return false;
        }
        return true;
    }
    ...
    for (const [key, value] of Object.entries(object)) {
        ...
        } else if (!equals(value, otherObject[key], copiedOptions)) return false;
    }
...
export function getByPath(object, keys) {
    ...
    for (let i = 0; i < keys.length; i++) {
        ...
        if (!hasOwnProperty(node, key)) {
            ...
            throw new ReferenceError(`No property found at ${JSON.stringify(currentPath)}`);
        }
        node = node[key];
    }
    return node;
...
export function getTransitiveClosure(object, propertyName) {
    ...
    const rv = new Set();
    let propertyValue = (hasOwnProperty(object, propertyName)
        ? object[propertyName]
        : undefined);
    ...
    for (const key in object) {
        if (hasOwnProperty(object, key)) {
            ...
            if (hasOwnProperty.call(rv, value)) {
                throw new TypeError("Given object is not invertible because its values are not unique.");
            }
            rv[value] = key;
        }
    }
...
export function setByPath(object, keys, value) {
    ...
    const parent = getByPath(object, [].slice.call(keys, 0, -1));
