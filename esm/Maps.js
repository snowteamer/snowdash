 ...
 * @returns {boolean}
 */
export function every(map, predicate, thisArg) {
    tc.expectMap(map);
 ...
 * @returns {Map<K, V>}
 */
export function filter(map, predicate, thisArg) {
    tc.expectMap(map);
    tc.expectFunction(predicate);
    const rv = new Map();
    for (const [key, value] of map) {
 ...
 * @param {*=} thisArg
 */
export function forEach(map, fn, thisArg) {
    tc.expectMap(map);
 ...
 * @returns {Map<K, V>}
 */
export function map(map, mapfn, thisArg) {
    tc.expectMap(map);
...
export function reduce(map, fn, initialValue) {
    tc.expectMap(map);
    tc.expectFunction(fn);
    if ("2" in arguments) return [...map].reduce((acc, [key, value]) => call(fn, undefined, acc, value, key, map), initialValue);
    return [...map].reduce((acc, [key, value]) => call(fn, undefined, acc, value, key, map));
}
 ...
 * @returns {boolean}
 */
export function hasEntry(map, entry) {
    tc.expectMap(map);
